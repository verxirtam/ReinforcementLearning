...
}
{
	...
	CostIntWidth2=TDist95Point[(N-1>100)?(100):(N-1)]*sqrt(CostUV/N);
	...
	#print CostSq/(N-1)" "CostMean*CostMean*N/(N-1)" "sqrt(CostUV/N);
	#print CostIntWidth2;
	#print CostUV" "N" "CostUV/N" "sqrt(CostUV/N);

	CountMean=CountSum/N;
	CountUV=CountSq/(N-1)-CountMean*CountMean*N/(N-1);
	CountIntWidth2=TDist95Point[(N-1>100)?(100):(N-1)]*sqrt(CountUV/N);
	...
	#print CountSq/(N-1)" "CountMean*CountMean*N/(N-1)" "CountUV" "sqrt(CountUV/N);
	print CostUV" "N" "CostUV/N" "sqrt(CostUV/N)" "CostIntWidth2" "CountUV" "N" "CountUV/N" "sqrt(CountUV/N)" "CountIntWidth2;
	#print CostIntWidth2" "CountIntWidth2;

	#print "["CostIntMin/CountIntMax", "CostIntMax/CountIntMin"]";
	#print CostIntMin"\t"CostIntMax"\t"CountIntMin"\t"CountIntMax"\t"CostIntMin/CountIntMax"\t"CostIntMax/CountIntMin;
	#################################################
}
END{
} 
